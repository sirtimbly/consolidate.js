...
exports.razor = function(path, options, cb) {
  return promisify(cb, function(cb) {
    ...
    if (!engine) {
      try {
        engine = requires.razor = require('razor');
      } catch (err) {
        try {
          engine = requires.razor = require('razor-templ');
        } catch (otherError) {
          throw err;
        }
      }
    ...
    try {
      ...
      // console.log('Razor template str: ' + str);
      var tf = engine(str);
      // console.log('Got a razor template fn: ' + tf);
      var tmpl = cache(options) || cache(options, function compiledRazorFn(locals) {
        // console.log('Executing the razor templating fn with locals : ', locals);
        return engine(tf, locals);
      });
      cb(null, tmpl(options));
    } catch (err) {
      cb(err);
